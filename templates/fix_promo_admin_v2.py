...
UPLOAD_DIR = STATIC_DIR / "uploads" / "promos"
...
ADMIN_EXTENSION = r'''
# ==============================
# Admin Management + Promotion Extension V2
# 홍보 신청 / 이미지 업로드 / 관리자 승인 / 계정 제재 / 통계 / 메인 팝업
# ==============================
...
def patch_app_py():
    ...
    if '"promoWhat"' not in text and "'promoWhat'" not in text:
        field_match = re.search(r"(FIELD_LIMITS\s*=\s*\{)", text)
        if field_match:
            insert_at = field_match.end()
            text = (
                text[:insert_at]
                + '\n    "promoWhat": 80,\n    "promoReason": 1000,\n    "adminNote": 300,'
                + text[insert_at:]
    ...
    if main_pos == -1:
        ...
        for marker in markers:
            ...
            if pos != -1 and pos < main_pos:
                ...
                cut_candidates.append(line_start if line_start != -1 else pos)

        if cut_candidates:
            cut_pos = min(cut_candidates)
            text = text[:cut_pos].rstrip() + "\n\n" + ADMIN_EXTENSION + "\n" + text[main_pos:]
        else:
            text = text[:main_pos].rstrip() + "\n\n" + ADMIN_EXTENSION + "\n" + text[main_pos:]
...
def patch_index_html():
    if not INDEX_PATH.exists():
        print("⚠️ templates/index.html을 찾지 못했습니다. 즐겨찾기 자동 추가는 건너뜁니다.")
        return

    text = INDEX_PATH.read_text(encoding="utf-8")

    promo_anchor_pattern = re.compile(
        r'\s*<a\s+href=[\'"]\/promo\/request[\'"]\s+class=[\'"]shortcut-item[\'"]>[\s\S]*?<\/a>',
        re.IGNORECASE
    )
    text = promo_anchor_pattern.sub("", text)
...
'''
    ...
    insert_pattern = re.compile(
        r'(\s*</div>\s*</section>\s*<section id="resultsArea")',
        re.IGNORECASE
    )

    if insert_pattern.search(text):
        text = insert_pattern.sub("\n" + promo_shortcut + r"\1", text, count=1)
    else:
        grid_pattern = re.compile(r'(<div class="shortcut-grid">[\s\S]*?)(</div>)', re.IGNORECASE)
        if grid_pattern.search(text):
            text = grid_pattern.sub(r"\1" + promo_shortcut + r"\2", text, count=1)
        else:
            print("⚠️ shortcut-grid를 찾지 못했습니다. index.html에서 홍보 신청 즐겨찾기를 직접 확인해주세요.")

    INDEX_PATH.write_text(text, encoding="utf-8")


def write_template_and_static_files():
...
def main():
    ...
    backup_dir = BASE_DIR / f"backup_before_promo_admin_v2_{timestamp}"
    ...
    patch_app_py()
    patch_index_html()
    write_template_and_static_files()

    print("✅ 업데이트 완료")
    print("✅ 홍보 신청 즐겨찾기 추가")
    print("✅ 홍보 이미지 5MB 제한 적용")
    print("✅ 관리자 승인/거절 버튼 API 연결 완료")
    print("✅ 계정 제재/삭제 버튼 API 연결 완료")
    print("✅ 최근 사용 기록 막대그래프 적용")
    print("✅ 승인된 홍보물 팝업 클릭 시 상세 페이지 이동 적용")
